...
capture_workload_b_failure_diagnostics() {
  ...
  for agent in block-a block-b block-c; do
    ...
    if [[ "$ip" =~ ^[0-9.]+$ ]]; then
      ...
      ssh_vm "$ip" "sudo grep -E '\"message\":\"(agent command received|command accepted|command acceptance rejected|command execution completed|command execution failed|create failed definitively; reporting terminal failure)\"' /var/log/o3k-compute.log 2>/dev/null | tail -n 64" \
...
install_agent() {
  ...
  ssh_vm "$ip" "sudo install -d -m 0750 /etc/o3k/tls /var/lib/o3k-compute; sudo install -m 0755 '$remote_stage/o3k-compute' /usr/local/bin/o3k-compute; sudo install -m 0644 '$remote_stage/ca.pem' /etc/o3k/tls/ca.pem; sudo install -m 0644 '$remote_stage/agent.pem' /etc/o3k/tls/agent.pem; sudo install -m 0600 '$remote_stage/agent-key.pem' /etc/o3k/tls/agent-key.pem; sudo install -m 0644 '$remote_stage/agent-id' /var/lib/o3k-compute/agent-id; sudo sh -c 'RUST_LOG=o3k_compute_agent=warn O3K_COMPUTE_CONTROL_ENDPOINT=https://o3k-control-plane:$CONTROL_PORT O3K_COMPUTE_SERVER_NAME=o3k-control-plane O3K_COMPUTE_TLS_DIR=/etc/o3k/tls O3K_COMPUTE_DATA_DIR=/var/lib/o3k-compute O3K_COMPUTE_HOST_LABEL=${id}-host O3K_COMPUTE_HEALTH_ADDR=127.0.0.1:19101 O3K_COMPUTE_MAX_DISK_GB=10 nohup /usr/local/bin/o3k-compute >/var/log/o3k-compute.log 2>&1 &'" \
