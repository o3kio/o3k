...
grep -Fq 'PP5 fast gate source mismatch' "${helper}"
grep -Fq 'requires a clean tracked tree' "${helper}"
grep -Fq 'git status --porcelain=v1 --untracked-files=all' "${helper}"
grep -Fq 'rejects untracked source/harness path' "${helper}"
