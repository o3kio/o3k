...
wait_server_active_cli() { # name -> id ; waits for ACTIVE via OpenStack CLI
  ...
  for i in $(seq 1 180); do
    out="$(openstack server list -f json --name "$name")" \
      || die "wait_server_active_cli($name): OpenStack CLI list failed (cannot verify)"
    id="$(python3 -c 'import json,sys; r=json.load(sys.stdin); print(r[0]["ID"] if r else "")' <<<"$out")" \
...
wait_server_absent_cli() { # name
  local name="$1" i id out
  for i in $(seq 1 60); do
    out="$(openstack server list -f json --name "$name")" \
      || die "wait_server_absent_cli($name): OpenStack CLI list failed (cannot verify absence)"
    id="$(python3 -c 'import json,sys; r=json.load(sys.stdin); print(r[0]["ID"] if r else "")' <<<"$out")" \
      || die "wait_server_absent_cli($name): unparseable CLI JSON"
    [ -z "$id" ] && return 0
    sleep 5
  done
...
print(d.get("id") or d.get("operationId") or d.get("operation_id") or "")' \
  ...
  *)
    ...
    python3 - "$EVID/21b-scenarioA-native-create-attempt.json" <<'PY' || die "the native create failure is not a canonical upstream error"
import json, sys
...
ORPHANS="$(python3 - "$EVID/21b-compute-instances-before.txt" "$EVID/21b-compute-instances-after.txt" <<'PY'
import sys
...
)" || die "cannot compute the compute_instance delta"
{
  ...
  echo "orphan_rows_created=$ORPHANS"
  echo "orphan_rows (id, observed_state, provider_id):"
  cat "$EVID/21b-compute-instances-after.txt.delta"
} > "$EVID/21c-scenarioA-orphan-rows.txt"
if [ "$ORPHANS" -gt 0 ]; then
  python3 - "$EVID/21b-compute-instances-after.txt.delta" <<'PY' || die "the create failure left a row that is NOT an orphan (it has a provider)"
import sys
rows = [line.rstrip("\n").split("\t") for line in open(sys.argv[1], encoding="utf-8") if line.strip()]
for row in rows:
    assert len(row) == 3, row
    assert row[2] == "", f"row {row[0]} carries provider {row[2]}: it is not an orphan"
PY
fi
...
record_gap "native-vm-create=network-provider-inactive" \
  "http_status=${CREATE_STATUS} operation_state=${CREATE_STATE} orphan_compute_instance_rows=${ORPHANS} o3k-network=$(systemctl is-active o3k-network 2>&1 || true)"
CURRENT_CASE=A-gap CURRENT_NAME="native compute.server create is NOT supported on this profile (classified gap)" \
  case_ok A-gap "native create FAILED as expected (http=$CREATE_STATUS, operation=$CREATE_STATE): no ACTIVE pp4-native, ${ORPHANS} orphan compute_instance row(s) recorded, cause lines captured in 21d (NOT a create)"
...
python3 - "$EVID/15-scenarioC-operations.json" "$B_ID" <<'PY' || die "scenario C: no stop Operation for pp4-openstack in the Araf operations list"
import json, sys
doc = json.load(open(sys.argv[1]))
resource_id = sys.argv[2]
items = doc.get("items", [])
def rid(item):
    return item.get("resourceId") or item.get("resource_id")
for_resource = [i for i in items if rid(i) == resource_id]
assert for_resource, f"no Operation references resource {resource_id}: {[rid(i) for i in items]}"
stop_ops = [i for i in for_resource if "stop" in str(i.get("action", "")).lower()]
assert stop_ops, f"no stop Operation for {resource_id}: {[i.get('action') for i in for_resource]}"
PY
...
for service in services:
    for rt in service.get("resource_types", []):
        if rt.get("id") == "compute.server":
            found = sorted(action.get("id") for action in rt.get("supported_actions", []))
...
for service in services:
    for rt in service.get("resource_types", []):
        if rt.get("id") == "compute.server":
            actions = sorted(action.get("id") for action in rt.get("supported_actions", []))
...
# CLI start -> Araf sees the workload ready again.
openstack server start "$B_ID" || die "scenario C: openstack start failed"
STARTED_STATUS="$(wait_bff_status compute.server "$B_ID" ready)"
...
grep -q 'No Server found' <<<"$CLI_OUT" \
...
openstack server create --wait --image "$IMAGE_ID" --flavor "$FLAVOR_ID" \
  --nic "net-id=$NET_ID" --config-drive true --key-name testlab-keypair pp4-openstack-cli \
  > "$EVID/17d-scenarioD2-cli-create.txt" || die "scenario D2: openstack server create failed"
...
wait_server_absent_cli pp4-openstack-cli
...
grep -q 'No Server found' <<<"$SRV_OUT" \
...
CROSS_SURFACE_STATUS="$(pcurl_status -H "cookie: ${OP_COOKIE}; ${OP_CSRF}" \
  https://operator.o3k.demo/api/v1/operator/platform/overview)"
{
  ...
  echo "tenant_session_on_operator_bff_operator_route: $(pcurl_status -H "cookie: ${BFF_COOKIE}; ${BFF_CSRF}" https://operator.o3k.demo/api/v1/operator/platform/overview)"
  echo "operator_session_on_operator_bff_operator_route: ${CROSS_SURFACE_STATUS}"
...
case_ok S1 "tenant/operator trust surfaces separated (tenant BFF 404 on operator route, tenant session rejected 401 by operator BFF)"
