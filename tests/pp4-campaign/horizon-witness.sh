...
curl -s -c "$JAR" -b "$JAR" -o "$LOGIN_RESP" -w '%{http_code}' \
  ...
  --data-urlencode "domain=Default" \
  --data-urlencode "region=RegionOne" \
  http://127.0.0.1:18091/auth/login/ | tee -a "$SUMMARY" >/dev/null
if grep -q 'Log Out' "$LOGIN_RESP" 2>/dev/null || curl -s -b "$JAR" http://127.0.0.1:18091/identity/ 2>/dev/null | grep -q 'Log Out'; then
