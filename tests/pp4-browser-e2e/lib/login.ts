...
export async function loginToSurface(
  ...
  expect(
   ...
   // The BFF serializes camelCase (`userId`/`userName`); tolerate the
   // snake_case shape as well so a serialization change is caught by the
   // truthiness assertion rather than a type error.
   expect(
     session.userName ?? session.user_name,
     "session must carry the federated user name",
   ).toBeTruthy();
...
export async function assertIdentityVisible(auth: AuthenticatedSurface): Promise<void> {
  // The console shell labels the signed-in *O3K principal* in its utility menu
  // (the federated, scoped identity — e.g. "admin"), while the IdP identity
  // ("Alice Demo") is asserted through the BFF session in loginToSurface. Both
  // are recorded: the shell must render the session-aware utility menu.
  const utility = auth.page
    .getByRole("link", { name: /^(User|Operator) menu for .+/ })
    .or(auth.page.getByRole("button", { name: /^(User|Operator) menu for .+/ }));
  await expect(
    utility.first(),
    `the ${auth.surface} shell must render the signed-in utility menu`,
  ).toBeVisible({ timeout: 30_000 });
  const label =
    (await utility.first().getAttribute("aria-label")) ??
    ((await utility.first().textContent()) ?? "");
  // eslint-disable-next-line no-console
  console.log(
    `[pp4] ${auth.surface} shell identity: "${label.trim()}" ` +
      `(IdP identity "${auth.userName}" asserted via the BFF session)`,
  );
}
