...
import { bffFetch, listOperations, listOperationsOrNull, logCookieNames, logout } from "../lib/bff";
import {
...
test("installed services show the discovered platform", async () => {
  ...
  const loadError = auth.page.getByText(/Could not load services/i);

  // The page either renders the discovered services or a truthful load error
  // (the pinned Araf release rejects O3K's descriptor because its action
  // `output` carries a canonical contract URL). Wait for whichever settles.
  const settled = await expect
    .poll(
      async () => {
        if ((await loadError.count()) > 0) return "error";
        if ((await servicesTable.count()) > 0) {
          const rows = await servicesTable.getByRole("row").count();
          if (rows > 1) return "table";
        }
        return "pending";
      },
  ...
  if (settled === "error" || settled === "pending") {
    await expect(
      loadError.first(),
      "installed services must render rows or the truthful descriptor error",
    ).toBeVisible({ timeout: 30_000 });
    const detail = (await loadError.first().locator("..").textContent()) ?? "";
    // eslint-disable-next-line no-console
    console.log(
      `PP4-GAP operator-installed-services-descriptor-rejected ` +
        `installed-services read failed truthfully: ${detail.trim().slice(0, 240)}`,
    );
    await expectNoFixtureMarkers(auth.page);
    return;
  }
...
test("regions render the deployment's region truth", async () => {
  await auth.page.goto(`${env.operatorUrl}/platform/regions`);
  await expect(
    auth.page.getByRole("heading", { name: "Regions", exact: true }).first(),
  ).toBeVisible({ timeout: 30_000 });

  const table = auth.page.getByRole("table", { name: "Regions table" });
  await expect(table).toBeVisible();
  const emptyState = auth.page.getByText(/No region data available/i);
  const dataRows = table
    .getByRole("row")
    .filter({ hasNot: auth.page.getByRole("columnheader") })
    .filter({ hasNotText: /No region data available/i });
  const rowCount = (await emptyState.count()) > 0 ? 0 : await dataRows.count();
  if (rowCount === 0) {
    // This deployment advertises no regions to the operator surface
    // (`GET /api/v1/operator/regions` returns []); the console states that
    // truthfully instead of inventing a region.
    await expect(
      emptyState.first(),
      "an empty region inventory must be stated explicitly",
    ).toBeVisible({ timeout: 30_000 });
    // eslint-disable-next-line no-console
    console.log(
      "PP4-GAP operator-regions-empty GET /api/v1/operator/regions returned [] " +
        "on this deployment; the console renders the explicit empty state",
    );
  } else {
    const cells = await dataRows
      .first()
      .getByRole("cell")
      .allTextContents();
    expect(cells.length, "a region row must render its columns").toBeGreaterThan(2);
    // eslint-disable-next-line no-console
    console.log(`[pp4] operator regions: ${rowCount} row(s); first: ${cells.join(" | ")}`);
  }
  await expectNoFixtureMarkers(auth.page);
});
...
test("operator operations tie the tenant journey to the canonical authority", async () => {
  ...
  if (operations === null) {
    ...
    // this deployment's operator surface; the page above stated that
    // truthfully and rendered no rows. Record the gap and stop here — the
    // canonical tie-in is proven on the tenant surface (its Operations view
    // shows the same canonical Operation the console delete produced).
    // eslint-disable-next-line no-console
    console.log(
      "PP4-GAP operator-canonical-operations-route-unavailable " +
        "GET /api/v1/operations is not mounted on the operator surface " +
        "(and /api/v1/operator/operations is HTTP 501)",
    );
    await expectNoFixtureMarkers(auth.page);
    return;
  }
