...
#     line) resolves to the pinned v0.4.0-rc.8 release asset path;
...
run_full_matrix() {
  ...
  SHIM_BIN="$MATRIX/bin"
  SRC_BUNDLE="$MATRIX/src-bundle/o3k-0.4.0-rc.8"
  TMP_ROOT="$MATRIX/tmp"
  WWW="$MATRIX/www"
  mkdir -p "$SHIM_BIN" "$SRC_BUNDLE/packaging" "$SRC_BUNDLE/bin" "$TMP_ROOT" \
    "$WWW/releases/v0.4.0-rc.8"
  printf 'ok\n' >"$WWW/ready"
...
if [ ! -e /usr/local/share/o3k/release-manifest.json ]; then
  ...
  printf '{"version":"0.4.0-rc.8","profile":"libvirt","source_commit":"fixture-source-sha-0000000000000000000000000000000000000000"}\n' \
...
case "$cmd" in
  ...
  tuple)
    ...
    printf 'O3K_VERSION=v0.4.0-rc.8\n'
...
EOF
  ...
  build_good_tarball() { # build_good_tarball TARBALL
    tar -C "$MATRIX/src-bundle" -czf "$1" ./o3k-0.4.0-rc.8
  }
  publish_tarball() { # publish_tarball TARBALL — copies into WWW + writes .sha256
    local digest
    cp "$1" "$WWW/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz"
    digest="$(sha256sum "$1" | awk '{print $1}')"
    printf '%s  %s\n' "$digest" "o3k-0.4.0-rc.8-linux-x86_64.tar.gz" \
      >"$WWW/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz.sha256"
  }
  publish_sha_digest() { # publish_sha_digest HEX64 — publishes a specific digest
    printf '%s  %s\n' "$1" "o3k-0.4.0-rc.8-linux-x86_64.tar.gz" \
      >"$WWW/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz.sha256"
  }
...
PY
  ...
  O3K_RELEASE_BASE="http://127.0.0.1:$DEAD_PORT/releases" \
    ...
    "download failed: http://127.0.0.1:$DEAD_PORT/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz" \
    "$out" "$err"
  ...
  #    OVERRIDE version's asset, not the baked v0.4.0-rc.8 one.
  ...
  rm -f -- "$WWW/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz" \
    "$WWW/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz.sha256"
  expect_abort "missing release asset (404) aborts" \
    "download failed: http://127.0.0.1:$RELEASE_PORT/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz" \
    "$out" "$err"
...
PY
  ...
  if [[ "$HEALTH_UP" -eq 0 ]]; then
    ...
    grep -Fq '✓ O3K v0.4.0-rc.8 verified' "$out" \
      && record_pass "baked default resolved to v0.4.0-rc.8 (verified banner)" \
      || record_fail "missing v0.4.0-rc.8 verified banner"
    if grep -Fq 'GET /releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz ' "$MATRIX/endpoint-http.log" \
      && grep -Fq 'GET /releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz.sha256 ' "$MATRIX/endpoint-http.log"; then
      record_pass "default resolution downloaded the pinned v0.4.0-rc.8 asset paths"
    else
      record_fail "release endpoint log does not show the pinned v0.4.0-rc.8 asset requests"
    fi
    ...
    grep -Fq 'Araf:' "$out" && grep -Fq '  version: v1.0.0-rc.12' "$out" \
      && grep -Fq '  source: de64cc9193085116fa30ad51c04ccab24a013dd0' "$out" \
  ...
  if [[ -e /usr/local/share/o3k/release-manifest.json || -L /usr/local/share/o3k ]]; then
    ...
    release_gets_before="$(endpoint_gets '/releases/v0.4.0-rc.8/')"
    expect_abort "upgrade fence refuses an implicit downgrade" \
      "installed v0.5.0-alpha.1 is newer than requested v0.4.0-rc.8; refusing implicit downgrade" \
      "$out" "$err"
    ...
    [[ "$(endpoint_gets '/releases/v0.4.0-rc.8/')" -eq "$release_gets_before" ]] \
    ...
    printf '#!/usr/bin/env sh\n# TEST FIXTURE install.sh release asset\n' \
      >"$WWW/releases/v0.4.0-rc.8/install.sh"
    if run_wrapper "$out" "$err"; then
    ...
    grep -Fq "Run: sudo $MATRIX/upgrade-download/o3k-0.4.0-rc.8/bin/o3k upgrade" "$out" \
    ...
    [[ -f "$MATRIX/upgrade-download/o3k-0.4.0-rc.8-linux-x86_64.tar.gz" \
      && -f "$MATRIX/upgrade-download/o3k-0.4.0-rc.8-linux-x86_64.tar.gz.sha256" \
    ...
    [[ -f "$MATRIX/upgrade-download/o3k-0.4.0-rc.8/bin/o3k" ]] \
      && record_pass "delegation staged the o3k entry point (no double nesting)" \
      || record_fail "staged entry point missing: $MATRIX/upgrade-download/o3k-0.4.0-rc.8/bin/o3k"
    ...
    (cd "$MATRIX/upgrade-download" && sha256sum -c --strict -- o3k-0.4.0-rc.8-linux-x86_64.tar.gz.sha256 >/dev/null) \
      && record_pass "delegated tarball matches the published SHA-256" \
      || record_fail "delegated tarball failed published SHA-256"
    cmp -s "$WWW/releases/v0.4.0-rc.8/install.sh" "$MATRIX/upgrade-download/install.sh" \
    ...
    [[ "$(endpoint_gets '/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz ')" -ge 1 ]] \
    ...
    tarball_gets_before="$(endpoint_gets '/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz ')"
    install_gets_before="$(endpoint_gets '/releases/v0.4.0-rc.8/install.sh ')"
    if run_wrapper "$out" "$err"; then
      record_pass "delegation re-run exits 0"
    else
      record_fail "delegation re-run failed (stderr: $(head -c 300 "$err"))"
    fi
    [[ "$(endpoint_gets '/releases/v0.4.0-rc.8/o3k-0.4.0-rc.8-linux-x86_64.tar.gz ')" -eq "$tarball_gets_before" ]] \
    ...
    [[ "$(endpoint_gets '/releases/v0.4.0-rc.8/install.sh ')" -gt "$install_gets_before" ]] \
    ...
    printf 'tampered\n' >>"$MATRIX/upgrade-download/o3k-0.4.0-rc.8-linux-x86_64.tar.gz"
...
if bash -c 'source "$1"; check_version_format v0.4.0-rc.8; check_version_format 0.4.0-alpha.1; check_version_format v0.2.0-alpha.1; check_version_format 1.2' \
  bash "$FUNCS" >/dev/null 2>&1; then
...
expect_compare "compare: equal versions (exit 1)" v0.4.0-rc.8 0.4.0-rc.8 1
