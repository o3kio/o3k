...
test('GET /version returns the alpha channel target', async () => {
  ...
  assert.equal(ALPHA_TARGET, 'v0.4.0-rc.3');
});

test('GET /channel/alpha returns v0.4.0-rc.3 as plain text', async () => {
  const response = await handler.fetch(request('GET', '/channel/alpha'));
  assert.equal(response.status, 200);
  assert.equal(response.headers.get('content-type'), 'text/plain; charset=utf-8');
  assert.equal(await body(response), 'v0.4.0-rc.3');
  // Intentional pin: see the version-constant comment on GET /version.
  assert.equal(CHANNELS.alpha, 'v0.4.0-rc.3');
});
...
test('GET /v0.4.0-rc.3 prepends the exact pin line and then equals the script', async () => {
  const response = await handler.fetch(request('GET', '/v0.4.0-rc.3'));
  assert.equal(response.status, 200);
  const text = await body(response);
  const expected = 'O3K_PINNED_VERSION="0.4.0-rc.3"\n' + REPO_SCRIPT;
  assert.equal(text, expected);
  assert.ok(text.startsWith('O3K_PINNED_VERSION="0.4.0-rc.3"\n'));
  assert.equal(text.slice('O3K_PINNED_VERSION="0.4.0-rc.3"\n'.length), REPO_SCRIPT);
});

test('GET /v0.4.0-rc.3 pin line is a plain sh assignment the wrapper parses', async () => {
...
test('GET /v0.4.0-rc.3/evil (traversal-shaped) is a 400, not a file read', async () => {
  const response = await handler.fetch(request('GET', '/v0.4.0-rc.3/evil'));
  assert.equal(response.status, 400);
});

test('unknown paths are 404 with a tiny helpful body', async () => {
  const response = await handler.fetch(request('GET', '/release/v0.4.0-rc.3'));
  assert.equal(response.status, 404);
