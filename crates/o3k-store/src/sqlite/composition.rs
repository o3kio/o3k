...
impl SqliteStore {
    pub async fn get_cloud_profile(
        ...
        let row = sqlx::query("SELECT profile_id,generation,payload,updated_at FROM cloud_profiles WHERE profile_id=?")
            .bind(id).fetch_optional(&self.pool).await.map_err(StoreError::Database)?;
        row.map(|r| {
    ...
    pub async fn upsert_cloud_profile(
        ...
        let current: Option<i64> =
            sqlx::query_scalar("SELECT generation FROM cloud_profiles WHERE profile_id=?")
                .bind(&p.profile_id)
        ...
        sqlx::query("INSERT INTO cloud_profiles(profile_id,generation,payload,updated_at) VALUES(?,?,?,?) ON CONFLICT(profile_id) DO UPDATE SET generation=excluded.generation,payload=excluded.payload,updated_at=excluded.updated_at")
            .bind(&p.profile_id).bind(i64::try_from(p.generation).map_err(|_| StoreError::Corrupt("profile generation overflow".into()))?).bind(&p.payload).bind(&p.updated_at).execute(&mut *tx).await.map_err(StoreError::Database)?;
    ...
    pub async fn delete_cloud_profile(&self, id: &str, expected: u64) -> Result<(), StoreError> {
        let result = sqlx::query("DELETE FROM cloud_profiles WHERE profile_id=? AND generation=?")
            .bind(id)
            .bind(
...
#[async_trait]
impl CompositionRepository for SqliteStore {
