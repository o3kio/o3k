...
fn record(row: &sqlx::postgres::PgRow) -> Result<BuildingBlockRecord, StoreError> {
...
impl PostgresStore {
    pub async fn get_building_block(
        ...
        sqlx::query("SELECT * FROM building_blocks WHERE block_id=$1")
            .bind(id)
    ...
    async fn upsert_block(
        ...
        let current: Option<i64> = sqlx::query_scalar(
            "SELECT generation FROM building_blocks WHERE block_id=$1 FOR UPDATE",
        )
        .bind(&block.id)
...
#[async_trait]
impl BuildingBlockRepository for PostgresStore {
